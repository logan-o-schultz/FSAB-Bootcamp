...
app.post("/posts", async (req, res) => {
    ...
    const collection = db.collection("posts");
    const newPost = {imageURL: postBodyData.imageURL, title: postBodyData.title, body: postBodyData.body, createdAt: (new Date().toString)};
    try {
...
function start() {
    const client = new MongoClient("mongodb+srv://admin3:abcdefg54321@cluster0.qzhjp.mongodb.net/myFirstDatabase?retryWrites=true&w=majority");
    client.connect()
        .then(() => {
